...
public class UtilGenerics {
	...
	/**
	 * Returns the Object argument as a parameterized List if the Object argument is
	 * an instance of List. Otherwise returns null.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Returns the Object argument as a parameterized Map if the Object argument is
	 * an instance of Map. Otherwise returns null.
	 */
	@SuppressWarnings("unchecked")
	...
	public static <K, V> Map<K, V> toMap(Class<K> keyType, Class<V> valueType, Object... data) {
		...
		for (int i = 0; i < data.length;) {
			...
			if (key != null && !(keyType.isInstance(key)))
				throw new IllegalArgumentException(
						"Key(" + i + ") is not a " + keyType.getName() + ", was(" + key.getClass().getName() + ")");
			i++;
			Object value = data[i];
			if (value != null && !(valueType.isInstance(value)))
				throw new IllegalArgumentException(
						"Value(" + i + ") is not a " + keyType.getName() + ", was(" + key.getClass().getName() + ")");
			i++;
