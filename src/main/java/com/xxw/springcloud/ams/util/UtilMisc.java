...
public class UtilMisc {
	...
	public static Logger logger = LoggerFactory.getLogger(UtilMisc.class);
	...
	public static <V, V1 extends V> Map<String, V> toMap(String name1, V1 value1) {
		...
		/*
		 * Map fields = FastMap.newInstance(); fields.put(name1, value1); return fields;
		 */
	}
	 ...
	 * @return The resulting Map
	 */
	public static <V, V1 extends V, V2 extends V> Map<String, V> toMap(String name1, V1 value1, String name2,
			V2 value2) {
		return new UtilMisc.SimpleMap<V>(name1, value1, name2, value2);

		/*
		 * Map fields = FastMap.newInstance(); fields.put(name1, value1);
		 * fields.put(name2, value2); return fields;
		 */
	}
	...
	public static <V, V1 extends V, V2 extends V, V3 extends V> Map<String, V> toMap(String name1, V1 value1,
		...
		/*
		 * Map fields = FastMap.newInstance(); fields.put(name1, value1);
		 * fields.put(name2, value2); fields.put(name3, value3); return fields;
		 */
	}
	...
	public static <V, V1 extends V, V2 extends V, V3 extends V, V4 extends V> Map<String, V> toMap(String name1,
		...
		/*
		 * Map fields = FastMap.newInstance(); fields.put(name1, value1);
		 * fields.put(name2, value2); fields.put(name3, value3); fields.put(name4,
		 * value4); return fields;
		 */
	}
	...
	/**
	 * This change a Map to be Serializable by removing all entries with values that
	...
	public static <V> void makeMapSerializable(Map<String, V> map) {
		...
		// for (Map.Entry<String, V> mapEntry: map.entrySet()) {
		// Debug.logInfo("Entry in Map made serializable: " + mapEntry.getKey() + "=" +
	...
	/**
	 * Converts an <code>Object</code> to a <code>double</code>. Returns zero if
	...
	/**
	 * Converts an <code>Object</code> to an <code>int</code>. Returns zero if
	 * conversion is not possible.
	 * 
	 * @param obj
	 *            Object to convert
	...
	/**
	 * Converts an <code>Object</code> to a <code>long</code>. Returns zero if
	 * conversion is not possible.
	 * 
	 * @param obj
	 *            Object to convert
	...
	/**
	 * Adds value to the key entry in theMap, or creates a new one if not already
	 * there
	 * 
	 * @param theMap
	 * @param key
	...
	public static Locale parseLocale(String localeString) {
		...
		if (localeString.length() == 2) {
			...
			String language = StringUtils.substring(localeString, 0, 2);
			String country = StringUtils.substring(localeString, 3, 5);
			locale = new Locale(language, country);
		} else if (localeString.length() > 6) {
			// positions 0-1 language, 3-4 are country, 6 and on are special extensions
			String language = StringUtils.substring(localeString, 0, 2);
			String country = StringUtils.substring(localeString, 3, 5);
			String extension = StringUtils.substring(localeString, 6);
			locale = new Locale(language, country, extension);
		} else {
			logger.warn("Do not know what to do with the localeString [" + localeString
	...
	protected static class SimpleMap<V> implements Map<String, V>, java.io.Serializable {
		...
		public int hashCode() {
			if (realMapIfNeeded != null) {
				...
				int hashCode = 0;
				for (int i = 0; i < names.length; i++) {
					// note that this calculation is done based on the calc specified in the Java
