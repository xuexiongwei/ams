...
public class ExcelListener extends AnalysisEventListener<Object> {
	...
	public ExcelListener (FileMapper f) {
		fileMapper = f;
	}
   ...
   public void invoke(Object object, AnalysisContext context) {
	   ...
	   try {
		   ...
		   if(no == 1) {//项目基本信息
			   ...
			   Xmjbxx jbxx = fileMapper.queryXmjbxx(prjSN);
			   if(null == jbxx)
				   jbxx = new Xmjbxx();
			   
			   BeanUtils.setProperty(jbxx, new String[]{"prjSN","prjUnit","prjAdr","prjName","prjType","contacts","contactInf","prjTemSN",
					   "specialNotifi","noticeTime","effectiveTime","remark"},
					   items);
			   if(null ==jbxx.getId())
				   fileMapper.saveXmjbxx(jbxx);
			   else
				   fileMapper.updateXmjbxx(jbxx);
		   }
		   
		   if(no == 2) {//项目属性
			   Xmsx sx = new Xmsx();
			   BeanUtils.setProperty(sx, new String[]{"prjSN","serialNumber","prjNature","prjAttr","peacetimeUses","aboveGroundLev","underGroundLev",
					   "aboveGroundHet","underGroundHet","buildings","housingStockNum","strucType","checkDocSN","checkDocDate","checkSN",
					   "checkDate","delaySN","delayCountDay","cancelSN","cancelDate","correctionSN","correctionDate","imgJudgeRes","exproprInfo",
					   "remark"},
					   items);
			   if(null != sx.getPrjSN())
				   fileMapper.saveXmsx(sx);
		   } 
		   
		   if(no == 3) {//项目明细
			   Xmmx mx = new Xmmx();
			   BeanUtils.setProperty(mx, new String[]{"prjSN","serialNumber","serialFunct",
			   ...
			   if(null !=mx.getPrjSN()) {
				   ...
				   for (Object obj : levs) {
					   ...
					   params.put("name", obj);
					   dic = fileMapper.queryDicByName(params);
					   if(null !=dic) {
						   parentID = dic.getId();
						   code = dic.getCode();
					   }
		   ...
		   if(no == 4) {//分类体系设计
			   ...
			   for (int i=0;i<10;i++) {
				   ...
				   if(null==curentID) {
					   ...
					   dic = fileMapper.queryDicByCode2(params);
					   boolean isNew = false;
					   if(null == dic) {
						   isNew = true;
						   dic = new ClassifiDic();
					   ...
					   if(isNew)
						   fileMapper.saveDic(dic);
					   else
						   fileMapper.updateDic(dic);
