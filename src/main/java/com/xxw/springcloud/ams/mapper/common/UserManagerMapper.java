...
public interface UserManagerMapper {
	...
	@Insert({"<script>",
			"INSERT INTO ams_sys_user_role(userId,roleId,user_create,user_modified) ",
			"VALUES ",
			"<foreach collection='userrole' item='item' index='index' separator=','>",
			"(#{item.userId},#{item.roleId},#{item.userCreate},#{item.userModified})",
			"</foreach>",
			"</script>"})
	 ...
	 * @param user
	 */
	@Update("UPDATE ams_sys_user SET password=#{password}"
			+ " WHERE id =#{id}")
